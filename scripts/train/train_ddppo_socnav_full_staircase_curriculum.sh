...
#SBATCH --output=slurm_logs/train/socialnav-ddppo-full-curriculum-staircase-%j.out
#SBATCH --error=slurm_logs/train/socialnav-ddppo-full-curriculum-staircase-%j.err
#SBATCH --gpus a40:4
...
# wandb config
JOB_ID="socnav_ddppo_baseline_multi_gpu_full_staircase_curriculum"
# split="train"
...
srun python -um socnav.run \
    --config-name=experiments/ddppo_socnav_full_staircase_curriculum.yaml \
    habitat.gps_available_every_x_steps=1 \
    habitat.curriculum_config.update_curriculum_every_x_steps=10000000 \
    habitat.curriculum_config.staircase_increment=50 \
