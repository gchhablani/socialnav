...
from habitat_baselines.rl.ppo.habitat_evaluator import HabitatEvaluator, pause_envs
...
class ZeroGpsEvaluator(HabitatEvaluator):
    ...
    def evaluate_agent(
        ...
        if len(config.habitat_baselines.eval.video_option) > 0:
            ...
            rgb_frames: List[List[np.ndarray]] = [
                [
                    observations_to_image(
                        {k: v[env_idx] for k, v in batch.items() if k !="step_id"}, {}
                    )
                ]
            ...
            for i in range(n_envs):
                ...
                if len(config.habitat_baselines.eval.video_option) > 0:
                    ...
                    frame = observations_to_image(
                        {k: v[i] for k, v in batch.items() if k!="step_id"}, disp_info
                    )
                    if not not_done_masks[i].any().item():
                        # The last frame corresponds to the first frame of the next episode
                        # but the info is correct. So we use a black frame
                        final_frame = observations_to_image(
                            {k: v[i] * 0.0 for k, v in batch.items() if k!="step_id"},
                            disp_info,
                        )
